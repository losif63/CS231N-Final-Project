...
class GeometryDashCNN(nn.Module):
    def __init__(
        ...
        Args:
            ...
            freeze_backbone: If True, freeze all backbone weights and only train the FC head.
        """
        super().__init__()
        ...
        if freeze_backbone:
            # Freeze conv1 through layer3 (indices 0-6); leave layer4 (index 7) trainable
            for i, child in enumerate(self.backbone.children()):
                if i < 7:
                    for param in child.parameters():
                        param.requires_grad = False

        # Attention pooling: learns a scalar score per frame, then takes weighted sum
    ...
    def forward(self, frames_list: list[torch.Tensor]) -> torch.Tensor:
        ...
        for frames in frames_list:
            ...
            weights = torch.softmax(self.attn(features), dim=0) # (T, 1)
            pooled.append((weights * features).sum(dim=0))      # (feature_dim,)
