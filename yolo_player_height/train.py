#!/usr/bin/env python3
"""
Train a YOLO-based model to predict player height and occlusion from
4-channel (RGB + motion diff) Geometry Dash gameplay images.
...
class GDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # Gaussian blur on all 4 channels (p = 0.3)
            if random.random() < 0.3:
                sigma = random.uniform(0.5, 1.5)
                for c in range(4):
                    crop[:, :, c] = cv2.GaussianBlur(crop[:, :, c], (3, 3), sigma)

        # ── Convert to normalised tensor [4, 620, 620] ────────────────────
        # Channels 0-2 are BGR; convert to RGB then apply ImageNet norm.
        bgr = crop[:, :, :3].astype(np.float32) / 255.0
        rgb = bgr[:, :, ::-1].copy()                             # BGR → RGB
        rgb = (rgb - IMAGENET_MEAN) / IMAGENET_STD              # ImageNet norm

        # Channel 3: greyscale frame diff, independent normalisation
...
def main():
    ...
    # Output directory
    run_dir = Path("runs") / datetime.now().strftime("train_%Y-%m-%d_%H-%M-%S")
