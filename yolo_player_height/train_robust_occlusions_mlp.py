#!/usr/bin/env python3
"""
OCCLUSION-FOCUSED training variant: Adds two extra augmentations for occlusion.

Compared to train_robust.py, adds:
  - Largest-rectangle crop (p=0.1): extract player-free region, resize to 620×620
  - Downscale-upscale (p=0.1): simulate compression artifacts on all frames
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if not occlusion and random.random() < 0.25:
                ...
                if rh > 0 and rw > 0:
                    side = min(rh, rw)
                    y_off = random.randint(0, rh - side)
                    x_off = random.randint(0, rw - side)
...
def _patch_classify_head(classify_layer):
    """
    Patch a YOLO Classify module in-place for regression with MLP head:
      - Replace the final Linear(1000) with an MLP: Linear(1280 → 128) → ReLU → Linear(128 → 2)
    ...
    # Initialize weights
    nn.init.xavier_normal_(mlp[0].weight)
    nn.init.xavier_normal_(mlp[2].weight)
    with torch.no_grad():
...
def run_epoch(model, loader, optimizer, device, train=True):
    ...
    return sum_loss / len(loader), sum_h / len(loader), sum_occ / len(loader), mae_px, occ_acc, occ_recall
...
def main():
    ...
    # Output directory
    run_dir = Path("runs") / datetime.now().strftime("train_robust_occlusions_%Y-%m-%d_%H-%M-%S")
    ...
    print(f"\n{'Epoch':>6}  {'Tr-Loss':>8}  {'Tr-H':>7}  {'Tr-Occ':>7}  "
          f"{'Va-Loss':>8}  {'Va-H':>7}  {'Va-Occ':>7}  {'Va-H-MAE':>8}  {'Va-Occ-Acc':>10}  {'Va-Occ-Rec':>10}")
    print("─" * 100)

    for epoch in range(1, args.epochs + 1):
        tr_loss, tr_h, tr_occ, _, _, _       = run_epoch(
            model, train_loader, optimizer, device, train=True)
        va_loss, va_h, va_occ, va_mae, va_occ_acc, va_occ_rec  = run_epoch(
            model, val_loader, optimizer, device, train=False)
        scheduler.step()

        print(f"{epoch:>6}  {tr_loss:>8.4f}  {tr_h:>7.4f}  {tr_occ:>7.4f}  "
              f"{va_loss:>8.4f}  {va_h:>7.4f}  {va_occ:>7.4f}  {va_mae:>8.1f}  {va_occ_acc:>10.1%}  {va_occ_rec:>10.1%}")
